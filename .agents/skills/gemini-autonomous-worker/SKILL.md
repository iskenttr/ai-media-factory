...
description: Run bounded autonomous software-engineering tasks with Gemini through Vertex AI on Google Compute Engine. Use when AI Media Factory needs background issue-to-branch implementation, structured patch generation, repair, test evidence, token/cost accounting, or a draft-PR-ready handoff while preserving isolated worktrees, human approval, and production boundaries.
---

# Gemini Autonomous Worker

Use Gemini as an untrusted proposal worker through the direct Vertex `generateContent` API. Keep Git preparation, patch application, tests, security review, commit, push, and acceptance under the deterministic orchestrator.
...
3. Verify the broker runs on Compute Engine with metadata-server ADC from the dedicated service account. Never create or load a service-account JSON key.
...
- Call Vertex `generateContent` with a hard abort timeout and a required JSON response schema.
- Do not send a `tools` field. The model receives only the trusted prompt context and cannot inspect files, browse, execute commands, or mutate state.
- Keep the root-owned `agent/runtime/gemini-settings.json` as defense in depth for any operator use of Gemini CLI; built-in tools use an empty allowlist and extensions, MCP, skills, and YOLO are disabled.
