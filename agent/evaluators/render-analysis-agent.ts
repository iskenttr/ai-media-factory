...
export async function renderSmokeArtifact(root: string, task: EngineeringTask, worktree: string, artifacts: string) {
  ...
  const render = await executeSandboxedCommand(root, worktree, artifacts, {
    ...
    argv: ["ffmpeg", "-nostdin", "-y", "-f", "lavfi", "-i", "color=c=0x20242b:s=320x180:d=1:r=25", "-vf", "subtitles=/artifacts/subtitles.ass", "-c:v", "mpeg4", "-pix_fmt", "yuv420p", "/artifacts/output.mp4"],
  });
  ...
  const frame = await executeSandboxedCommand(root, worktree, artifacts, {
    ...
    argv: ["ffmpeg", "-nostdin", "-y", "-ss", "0.5", "-i", "/artifacts/output.mp4", "-frames:v", "1", "/artifacts/sampled-frames/frame-001.png"],
  });
