...
export async function executeSandboxedCommand(root: string, worktree: string, artifacts: string, request: CommandRequest): Promise<CommandResult> {
  const validated = validateCommand(request, worktree, artifacts);
  ...
  const output: CommandResult = { argv, ...result, durationMs: Date.now() - started, stdoutPath, stderrPath, timedOut, outputLimitExceeded };
  await appendAudit(root, {
    timestamp: new Date().toISOString(), taskId: request.taskId, category: "command", event: "sandbox_command_finished",
    detail: { argv, cwd: request.cwd, exitCode: output.exitCode, signal: output.signal, durationMs: output.durationMs, stdoutPath, stderrPath, timedOut, outputLimitExceeded },
  });
  return output;
}
